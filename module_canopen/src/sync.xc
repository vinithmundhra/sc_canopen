                                              ...
                                              can_state_t can_state)
{
  ...
  {
    sync_timer[(int)pdo_number].sync_counter = sync_timer[(int)pdo_number].sync_counter
        + 1; //increment sync counter
    if ((sync_timer[(int)pdo_number].tx_data_ready == TRUE)
        && (sync_timer[(int)pdo_number].sync_value == 0)) //check if data avialable for transmit
    {
          ...
          == 0)) //check if tx is in sync window length
      {
        if ((tpdo_inhibit_time_values[(int)pdo_number].inhibit_time
            ...
            || (tpdo_inhibit_time_values[(int)pdo_number].inhibit_time == 0)) //check if inhibit time is reached
        {
    ...
    else if ((sync_timer[(int)pdo_number].sync_counter
        == sync_timer[(int)pdo_number].sync_value)
        && (sync_timer[(int)pdo_number].sync_value <= 240)) //sync messages
    {
      sync_window_timer:>sync_time_current;
      if (sync_time_start < sync_time_current)
      {
          ...
          == 0))
      {
        if ((tpdo_inhibit_time_values[(int)pdo_number].inhibit_time
            < tpdo_inhibit_time_values[(int)pdo_number].inhibit_counter)
            || (tpdo_inhibit_time_values[(int)pdo_number].inhibit_time == 0))
        {
          pdo_transmit_data(sync_timer[(int)pdo_number].comm_parameter,
                            sync_timer[(int)pdo_number].mapping_parameter,
                            c_rx_tx, can_state);
    ...
    else if ((sync_timer[(int)pdo_number].sync_value == 252)
        && (sync_timer[(int)pdo_number].tx_data_ready == TRUE)) //RTR request
    {
      sync_window_timer:>sync_time_current;
      if (sync_time_start < sync_time_current)
      {
...
{
  unsigned time_difference_sync;
  if ((sync_messages_rx[(int)pdo_number].rx_data_ready == TRUE)
      && (sync_messages_rx[(int)pdo_number].sync_value == 0))
  {
    sync_window_timer:>sync_time_current;
    if (sync_time_start < sync_time_current)
    {
    ...
    if (time_difference_sync < sync_window_length)
    {
      ...
      pdo_send_data_to_application(RPDO_0_MAPPING_PARAMETER + pdo_number,
                                   sync_messages_rx[(int)pdo_number].data,
                                   sync_messages_rx[(int)pdo_number].data_length,
                                   c_application);
    }
  }
  else
  {
    if (sync_messages_rx[(int)pdo_number].rx_data_ready == TRUE)
    {
      sync_messages_rx[(int)pdo_number].sync_counter
          = sync_messages_rx[(int)pdo_number].sync_counter + 1; //increment sync counter
      if (sync_messages_rx[(int)pdo_number].sync_counter
          == sync_messages_rx[(int)pdo_number].sync_value) //check if counter reached sync value
